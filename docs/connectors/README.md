...
mtime as ISO date, a `file://` URL, and the file extension as the document `type`.
`--recursive` includes nested subdirectories.
...
(`id`, `name`, `dateModified`, `webUrl`, plus the optional `type` — the store's MIME/kind
field, normalized to a friendly form like `spreadsheet` and stored as
`schema:additionalType`). The `query_syntax: google-drive` flag enables
