...
**This is a connections concern, not a prompts concern** — it wires MCP servers into Desktop exactly as `connections/servers.yaml` wires them into any other target. The MCP half of `targets/claude-app.yaml` lives on the `--lane connections` deploy lane (the skills half is on `--lane content`).
...
(`owned_keys: [mcpServers]`) — the same surgical-merge pattern as Antigravity's
`config.json`. Claude Desktop stores its own preferences in this *same* file, so a
