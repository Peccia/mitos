...
*correct, current* document instead of letting it guess.
...
`python build/compile.py review` opens a localhost web console with three tabs: **Inbox**
(review/accept/reject every candidate against a live diff), **Knowledge Graph** (propose
document mappings), and **Prompt Library** (browse, copy, and compose your registry prose for
one-shot use in any chat app). It edits the working tree and never commits — `git status` shows
