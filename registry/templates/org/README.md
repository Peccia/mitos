...
- `session-protocol.md` — the lean, always-on identity partial: the **Session Protocol**
  ...
  by last-layer-wins and flowing into Mitos Agent's `SOUL.md` automatically — which is why a
...
`org-marketing` — and are available on all mitos-agent environments without any per-user seeding.
