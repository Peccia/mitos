...
Handle {{users_given_name}} one-shot personal requests using the connected workspace tools.
Categorize the request — email, calendar, task, note/idea, or contact — then follow the
matching category's steps below.
...
## Tools
- When the request names a document or a project, resolve the name in the project maps
  first — `../Projects/AGENTS.md` for the roster, `../Projects/<project>/AGENTS.md` +
  `AGENTS_DETAILS.md` for document titles and IDs — and operate on the mapped ID. Search
  the document store by name only when the maps don't list it.
