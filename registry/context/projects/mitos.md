...
| A skill's supporting files (example outputs, executable scripts) | `registry/skills/<name>/examples/` and `.../scripts/` — auto-discovered at load (`loader._load_skill_resources`), UTF-8 text only (v1; a binary file fails loudly). Deployed alongside `SKILL.md` on hermes/claude-code (own per-file Outputs, inheriting the skill's drift policy) and bundled into claude-app zips (`Output.zip_members`). Each file's OWN path is its adopt/harvest source — an edited script routes back to itself, never `SKILL.md`. Console: the Skills & Orgs tab's Supporting Files panel (full-replacement-set semantics — see `docs/managing-state.md`) |
...
| Which prompts a tool receives | the prompt's `targets:` frontmatter (omit = console-only); the target's `prompts:` block in `targets/<tool>.yaml` selects them. Today only `gemini` deploys prompts (to `antigravity_skills/prompt-<name>.md`). |
...
| Which skills/agents a project's Claude Code checkout gets | `skills:`/`agents:` lists in `registry/projects/<slug>.yaml`; a bound skill must target `claude-code` or `gemini` (the two targets with a project-scoped skill surface — `PROJECT_SCOPE_CAPABLE_TARGETS`). Deployed to `<checkout>/.claude/skills/` and `.claude/agents/` (and `.agents/skills/` if the skill also targets `gemini`) |
...
| Add a custom branch to the assistant tree (e.g. `family/`) | drop an `AGENTS.md` under `registry/context/<branch>/` — any sibling file in that folder deploys alongside it to `<assistant_root>/<branch>/…`. Auto-discovered at plan time (`planner._plan_agents_md`, keyed off the existing `context/` partial scan — no loader change needed) and listed in a `<generated>` section on the root `AGENTS.md`. The only way to extend the tree without forking `targets/agents-md.yaml`, which is **not** overlayable. Branch names may not collide with a reserved top-level entry (`Projects`, `Assistant`) — rejected loudly at plan time |
