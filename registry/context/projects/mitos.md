...
| Which prompts a tool receives | the prompt's `targets:` frontmatter (omit = console-only); the target's `prompts:` block in `targets/<tool>.yaml` selects them. Today only `antigravity` deploys prompts (to `antigravity_skills/prompt-<name>.md`). |
