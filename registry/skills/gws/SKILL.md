...
### Gmail (read-only)
- **Search:** `search_gmail_messages` using Gmail operators — `from:`, `to:`, `subject:`, `is:unread`, `is:starred`, `label:`, `-label:`, `after:`/`before:`. Returns message IDs. `from:` accepts a bare name (`from:jody`); `before:` is exclusive — yesterday-through-today is `after:YYYY/MM/DD before:YYYY/MM/DD+1`.
