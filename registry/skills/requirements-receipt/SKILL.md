...
- **If a store is named above**, create a document there titled `<work item key> — requirements-receipt`,
  with this record's complete content — the `---` header included, exactly as you wrote it
  locally — and add the URL it returns as the `locator:` line above. The header is what tells the
  reader which work item and which run this document belongs to. Publish it even though the
  identity block below repeats it: the two carriers exist because either one can be lost to a
...
```json
{
  ...
  "identifier": "<run>",
  "http://peccia.net/deliverable": "requirements-receipt",
