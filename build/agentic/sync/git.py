...
# Path resolution + core.sshCommand construction live in `agentic.sshkey`, shared with
# commands.py's `repo_ssh_keys:` project-clone auth — kept as module aliases here so existing
# call sites (and tests addressing `gitsync._resolve_key_path`/`_ssh_command`) are unaffected.
_resolve_key_path = sshkey.resolve_key_path
_ssh_command = sshkey.ssh_command
