...
from .base import ConnectorError, WorkspaceConnector, friendly_doc_type
    ...
    def _map_rows(self, rows: list) -> list[dict]:
        ...
        for r in rows:
            ...
            out.append({
                ...
                # optional: only populated when graph_enum.fields maps `type` (e.g. to
                # the store's mimeType); absent mapping → "" and the graph omits it
                "type": friendly_doc_type(str(r.get(self._field("type"), "") or "")),
            })
        return out
