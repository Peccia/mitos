...
from .io import dump_json, expand, safe_rel, sha256, write_bytes, write_text, zip_bytes
...
def _payload(o: Output) -> bytes:
    ...
    archive bytes from the member text; everything else is UTF-8 text)."""
    if o.kind == "zip":
...
def cmd_deploy(reg: Registry, machine: str, dry_run: bool, force: bool,
    ...
    for s in statuses:
        ...
        if o.kind == "zip":
            ...
            write_text(_dest(o.deploy_path, root), o.content)
        written += 1
        h = sha256(payload)
        entry = {
...
def route_into_registry(reg: Registry, registry_path: str, payload_text: str,
    ...
    if sections:
        ...
        for src, new_body in mapping.items():
            ...
            if partial is None:
                ...
                continue
            if new_body != partial.body:
                _rewrite_registry_body(reg, src, new_body)
    ...
    if (m.group(2) if m else current).strip("\n") == body.strip("\n"):
        return [], [], None
    _rewrite_registry_body(reg, real, body.strip("\n"))
    return [real], [], None
