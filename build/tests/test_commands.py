...
def test_review_refuses_unroutable_candidates():
    ...
    # the synthetic section text ("alpha\nbeta"/"gamma") never matches the real partial
    # bodies, so this candidate also reads as stale (P0's gate) — force past that to
    # exercise the boundary-split failure this test actually targets.
    out = review.decide(treg, "t2--rig--doc", "accept", "", force=True)
