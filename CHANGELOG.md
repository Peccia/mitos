...
### Added
- Feature: The seven `delivers:` skills now publish their finished record to the project's shared connection, not just the local `{{returns_root}}` file. The publish step reads the machine's `{{connection}}` token so the harness names a real store instead of hunting for a connection heading in always-on context (a coding-only machine has a live `mcp.json` with nothing naming it, which is why the feature's first cut correctly refused to guess and published nothing). It publishes the FULL record — the `---` header included, since a body-only copy carries no `work:`/`run:` fields and reads to the operator as "nothing published" — and reads the copy back to report a store that reformats it. Each record's `## Identity` block now carries `schema`, `work`, `format` and `produced_by` alongside `run`/`deliverable`: a document imported into a store loses its `---` fence to a horizontal rule, so the block is the one carrier that survives that round trip; a work item key with no `__` still keeps the block, dropping only the `isPartOf` mapping rather than the whole thing. A new `return-record` identity variant (`docs/implemented-document-identity.md`) adds `http://peccia.net/deliverable` under the same bare schema.org context an effort's forward contract uses, so an effort declaring `tests` and a record answering `tests` share one vocabulary in both directions; `IDENTITY_TYPES` narrows to `implemented-requirements` — a run's records are Mitos-Agent's raw input for the one Implemented Document it produces, not project documents Mitos itself claims.
- Feature: Repo-root `mitos` (POSIX sh) and `mitos.cmd` shims resolve `build/.venv`'s interpreter and route interactive verbs (`init`, `project`, `connect`, `connectors`, `sync`) to `build/mitos.py`, everything else to `build/compile.py` — a new deterministic verb needs no shim edit. `test_cli_shim_verbs_match_mitos_py` guards against an interactive verb silently falling through to the compiler's `invalid choice` error.
- Feature: The console's "Tweak & map" flow prefills the effort selector from Mitos-Agent's identity fragment. A new `mitos.py peek` verb prints one staged document's raw content via its resolved connector (console-only, a subprocess like `connect`); the console scans the result for the JSON-LD identity block and degrades silently — no suggestion, never an error — on an unreachable store, unparseable JSON, wrong type, or an effort id the project's graph doesn't know. The editor opens immediately regardless of the network, and prefills the Parent selector only if a suggestion arrives before the operator has picked one themselves.
- Feature: `hidden: true` on a project manifest keeps a project's context out of every deployed tree — deliberately independent of `stage:`, since a `maintain`-staged project still needs its context deployed. Filters both `reg.projects` and `reg.graphs` (the agentic-graph reference tree reads `.graphs` directly, so filtering only one leaves a hidden project half-visible); `loader.load()` itself doesn't filter, so the manifest and graph still load and query fine outside planning. Editable from the console's Project panel (Hidden checkbox + summary badge).
- Feature: `keywords` on a `CreativeWork` effort (`schema:keywords`, JSON-LD) renders an `_Also known as: …_` line in generated markdown; project manifests gain an optional `aliases` field, validated for format/characters and rendered onto roster lines.
- Added a static AST-based guard (`test_boundary.py`) walking every module under `build/` and failing if any real import names `mitos_agent` — distinguishing actual imports from harmless string/identifier references (e.g. `mitos_agent_mcp_config`) — enforcing the "Mitos never depends on Mitos-Agent" architectural invariant in code rather than contributor discipline.
...
### Changed
- `claude-app` (a `mode: zip` manual target — the only kind, since claude.ai exposes no filesystem or upload API) no longer honors machine `include:`/`exclude:` curation: staging isn't deploying, and excluding a skill from a staged zip only cost the operator a registry edit and redeploy to get back a choice upload time already makes. Scope-leak warnings (`SCOPE_IGNORING_SKILL_TARGETS`) narrow to `mitos-agent` alone, since a staged zip is inert and has leaked nowhere — on windows-main this retires all 7 standing skill warnings with no deployment loss. `requires_server:` still gates a manual target; a curation block on one is now refused at load rather than silently ignored.
- Migrated skill frontmatter's legacy `hermes:` tag key to `mitos_agent:` across all 17 registry skills and dropped the `fm.get("hermes")` render fallback, along with the remaining Hermes references in loader/planner/render/review comments, `targets/mitos-agent.yaml`, `AGENTS.md`, docs, and six test modules — the legacy target retired in 0.1.5 but its name lived on in dozens of places.
- Internal: trimmed and deduplicated this repo's own builder context (split the Mitos project node from its `AGENTS.md` self-doc artifact, -29% on the largest reference table), refreshed onboarding docs (README primer, Quick Start, command reference), and closed a requirements-coverage documentation gap.
...
### Fixed
- Return records were landing in a project's watched folder and being ingested as project context — four runs rendered a finished implementation's claims into that project's generated `AGENTS.md` as always-on fact, while the evaluation folder meant to hold them stayed empty. The publish step named a store and no destination, so the harness picked the nearest folder it knew about. Added `returns_container` to a connection in `servers.yaml` — the folder id return records publish into and mitos-agent reads back, named once so writer and reader can't disagree — and the seven skills now refuse to guess a destination, least of all a project folder in the same store, when the token stays literal.
- An effort with no documents mapped rendered nothing — no goal, no expected deliverables, no requirements coverage — even though those are declared in the graph rather than derived from documents, and the deliverables line is documented as ungated in every generated view. `_doc_block` returned early on `not pg.documents` and `_grouped`'s `has_efforts` only asked whether an effort HELD a document; now only "no documents AND no efforts" short-circuits, and an empty-but-declared effort renders with an explicit "no documents in this effort" note.
- `mitos init`'s org-routing prompt told a new user the right org "activates per-project via each project's `org:` field" — no such field exists; `loader._validate` rejects `org:` on a manifest, and an org domain is tagged per EFFORT (`orgDomain`) in the knowledge graph, routed per task rather than per project. Setting up a Mitos Agent machine is exactly when a new user meets this prompt, so the wizard was teaching the one thing guaranteed to fail.
- Nested markdown lists in the operator console's preview rendered as stray text or a `<pre class="poetry">` block — snarkdown's indented-block rule matched ahead of its list rule, and nested lists are out of scope upstream. Replaced with `marked` 12.0.2 (still sanitized through `DOMPurify.sanitize()`, which does the work `marked` deliberately doesn't); the preview also gained GFM tables/task-list styling and list/heading/blockquote/pre/hr rules it previously had none of.
